...
describe("careerMoverLens + buildPlan", () => {
  ...
  it("ranks by recombined sub-scores; excludes unscored companies", async () => {
    await scoredCompany(db, "elite", { f: 0.9, i: 0.9, d: 0.9, rationale: "Ex-DeepMind founders" });
    await scoredCompany(db, "weak", { f: 0.2, i: 0.2, d: 0.2 });
    // No sub-scores AND no positive overall → excluded.
    await createCompanyRepo(db).create({ slug: "unscored", name: "unscored", status: "new", scoreOverall: null });

    const plan = buildPlan({ lens: careerMoverLens, profile: PROFILE, graph: await loadGraph(db), now: NOW });
  ...
  it("why-line leads with fit; timing clause only when the raise is recent + sourced", async () => {
    await scoredCompany(db, "fresh", { f: 0.9, i: 0.9, d: 0.9, round: "Series A", fundDate: "2026-05-01" });
    await scoredCompany(db, "stale", { f: 0.9, i: 0.9, d: 0.9, round: "Series B", fundDate: "2024-01-01" });
  ...
  it("every claim carries provenance; funding traces to Apollo, rationale to taste review", async () => {
    await scoredCompany(db, "acme", { f: 0.9, i: 0.8, d: 0.8, rationale: "good", round: "Seed", fundDate: "2026-05-01" });
    const plan = buildPlan({ lens: careerMoverLens, profile: PROFILE, graph: await loadGraph(db), now: NOW });
  ...
  it("nests who-to-meet: speakers first, with talk slot; opener references the speaker", async () => {
    const c = await scoredCompany(db, "yutori", { f: 0.9, i: 0.8, d: 0.9, rationale: "elite" });
    const people = createPersonRepo(db);
    const nonSpeaker = await people.create({ slug: "p1", name: "Quiet Person", companyId: c.id, relationship: "network_contact" });
    const speaker = await people.create({ slug: "dhruv", name: "Dhruv Batra", companyId: c.id, relationship: "network_contact", title: "Founder" });
    void nonSpeaker;
    await createTalkRepo(db).createIgnore({
  ...
  it("hiring garnish: open roles boost score and surface as claims + openRoles", async () => {
    const c = await scoredCompany(db, "hiring", { f: 0.8, i: 0.8, d: 0.8 });
    const roles = createRoleRepo(db);
    await roles.create({ companyId: c.id, title: "Founding Engineer", status: "new", source: "apollo", postedDate: "2026-06-10T00:00:00Z" });
    const plan = buildPlan({ lens: careerMoverLens, profile: PROFILE, graph: await loadGraph(db), now: NOW });
  ...
  it("respects the limit", async () => {
    for (let i = 0; i < 12; i++) await scoredCompany(db, `c${i}`, { f: 0.5 + i * 0.01, i: 0.5, d: 0.5 });
    const plan = buildPlan({ lens: careerMoverLens, profile: PROFILE, graph: await loadGraph(db), now: NOW, limit: 5 });
